...
##修復日期格式問題
    ...
    '__DATE_FILTER_FUTURE' => '過去',
    'AUTO DETECT' => '自動偵測',
    '_BLOG_CONFIG_MODE_BASIC' => '基本模式',
    '_BLOG_CONFIG_MODE_DETAIL' => '詳細模式',
    '_SEARCH_SIDEBAR' => '搜尋',
    '_USAGE_REBUILD' => '<a href="javascript:void(0)" onclick="doRebuild()">重建</a>以將這些變更反映到您的公開網站。',
    '_USAGE_FORGOT_PASSWORD_1' => '您已要求找回 Movable Type 密碼。系統已為您重設密碼；新密碼如下：',
    '_USAGE_FORGOT_PASSWORD_2' => '您可以透過下方 URL 使用此新密碼登入 Movable Type。登入後，請將密碼改為較易記住的內容。',
    '_BACKUP_DOWNLOAD_MESSAGE' => '匯出檔案將在幾秒後自動開始下載。若未自動開始，請點擊<a href="javascript:(void)" onclick="submit_form()">這裡</a>手動下載。請注意，每個工作階段只能下載一次匯出檔案。',
    '_USAGE_ARCHIVE_MAPS' => '這個進階功能可讓您將任意彙整模板對應到多種彙整類型。例如，您可能希望為每月彙整建立兩種不同檢視：一種以清單顯示該月文章，另一種以該月行事曆檢視呈現。',
    '_USAGE_ARCHIVING_1' => '請選擇您希望網站啟用的彙整頻率/類型。對每一種彙整類型，您都可以指定多個彙整模板。例如，您可能希望為每月彙整建立兩種不同檢視：一種頁面列出該月所有文章，另一種以該月行事曆檢視呈現。',
    '_USAGE_ARCHIVING_2' => '當您將多個模板關聯到某個彙整類型（甚至只關聯一個）時，都可以使用彙整檔案模板自訂彙整檔案輸出路徑。',
    '_USAGE_ARCHIVING_3' => '請選擇要新增彙整模板的彙整類型，然後選擇要與該彙整類型關聯的模板。',
    '_USAGE_BANLIST' => '下方列出您已禁止在網站留言或送出 TrackBack 的 IP 位址。要新增 IP，請在下方表單輸入位址；要刪除已封鎖 IP，請在下表勾選刪除並按下「刪除」按鈕。',
    '_USAGE_PREFS' => '此畫面可設定與部落格、彙整、留言，以及發佈與通知相關的各種選用項目。建立新部落格時，這些值會採用合理預設。',
    '_USAGE_FEEDBACK_PREFS' => '此畫面可設定讀者向您的部落格提供回饋的方式。',
    '_USAGE_PROFILE' => '在此編輯您的使用者資料。若您變更使用者名稱或密碼，登入憑證會自動更新，無需重新登入。',
    '_GENL_USAGE_PROFILE' => '在此編輯該使用者資料。若變更使用者名稱或密碼，該使用者的登入憑證會自動更新，無需重新登入。',
    '_USAGE_GROUP_PROFILE' => '此畫面可編輯群組資料。',
    '_USAGE_NEW_AUTHOR' => '您可以在此畫面建立系統新使用者。',
    '_USAGE_NEW_GROUP' => '您可以在此畫面建立系統新群組。',
    '_USAGE_ROLES' => '您可以在此畫面查看您在各部落格中的角色並建立角色。點擊角色名稱可查看詳細資訊。',
    '_USAGE_ROLE_PROFILE' => '您可以在此畫面定義角色及其權限。',
    '_USAGE_ASSOCIATIONS' => '您可以在此畫面查看並建立權限關聯。',
    '_USAGE_CATEGORIES' => '使用分類可更方便地整理文章，便於參考、彙整與網站呈現。建立或編輯文章時可指定分類。要編輯既有分類請點擊分類標題；要建立子分類請點擊對應「建立」按鈕；要移動分類請點擊對應「移動」按鈕。',
    '_USAGE_CATEGORY_PING_URL' => '這是其他人向您的部落格送出 TrackBack 時使用的 URL。若希望任何人都可向此分類送出 TrackBack，請公開此 URL；若僅允許特定對象，請私下提供此 URL。若要在主索引模板顯示收到的 TrackBack 清單，請參閱與 TrackBack 相關的模板標籤說明。',
    '_USAGE_TAGS' => '使用標籤可更方便地整理文章，便於參考與網站呈現。',
    '_USAGE_COMMENT' => '編輯所選留言。完成後按下「儲存」。要讓變更生效，您需要重建。',
    '_USAGE_PERMISSIONS_1' => '您正在編輯 <b>[_1]</b> 的權限。下方列出您有使用者編輯權限的部落格；請在每個部落格勾選要授予 <b>[_1]</b> 的存取權限。',
    '_USAGE_PERMISSIONS_2' => '若要編輯其他使用者權限，請在下拉選單選擇使用者，然後按下「編輯」。',
    '_USAGE_PERMISSIONS_3' => '您可用兩種方式編輯使用者並授與/撤銷存取權限。快捷方式是從下方選單選擇使用者並編輯；也可瀏覽完整使用者清單，再選擇要編輯或刪除的使用者。',
    '_USAGE_PERMISSIONS_4' => '每個部落格可有多位使用者。新增使用者時，請在下方表單輸入使用者資訊，並選擇該使用者可存取的部落格。按下「儲存」建立使用者後，您可繼續編輯其權限。',
    '_USAGE_PLACEMENTS' => '使用下方工具管理此文章的次要分類。左側清單為此文章尚未指定為主要或次要分類的分類；右側清單為此文章目前已指定的次要分類。',
    '_USAGE_ENTRYPREFS' => '選擇在文章編輯器中顯示的欄位集合。',
    '_USAGE_IMPORT' => '您可以從 Movable Type 安裝目錄下的 <code>import</code> 目錄檔案匯入部落格文章，也可透過下方表單上傳後匯入。支援從其他 Movable Type 安裝或其他應用程式匯入。',
    '_USAGE_EXPORT_2' => '要匯出文章，請點擊下方連結（「從 [_1] 匯出文章」）。若要將匯出資料儲存為檔案，Mac 請按住 <code>option</code> 鍵，PC 請按住 <code>Shift</code> 鍵再點擊連結。也可全選資料後複製到其他文件。（<a href="javascript:void(0)" onclick="openManual(\'importing\', \'export_ie\');return false;">從 Internet Explorer 匯出？</a>）',
    '_USAGE_EXPORT_3' => '點擊下方連結會將您目前所有部落格文章匯出到 Tangent 伺服器。通常這是在安裝 Movable Type 的 Tangent 外掛後進行的一次性推送，但您也可在需要時執行。',
    '_USAGE_AUTHORS' => '這是 Movable Type 系統中的全部使用者清單。點擊使用者名稱可編輯其資料。',
    '_USAGE_AUTHORS_1' => '這是 Movable Type 系統中的全部使用者清單。點擊使用者名稱可編輯其資料。您也可透過 CSV 指令檔建立、編輯與刪除使用者記錄。',
    '_USAGE_AUTHORS_LDAP' => '這是 Movable Type 系統中的全部使用者清單。點擊使用者名稱可編輯其資料。勾選使用者旁核取方塊並按下「停用」可停用該使用者，停用後其將無法登入 Movable Type。',
    '_USAGE_AUTHORS_2' => '您可以上傳包含指令與相關資料的 CSV 格式檔案，以批次建立、編輯與刪除使用者。',
    '_USAGE_GROUPS' => '下方是 Movable Type 系統中的全部群組。勾選群組名稱旁核取方塊後按下「啟用」或「停用」可啟用/停用群組；點擊群組名稱可編輯群組。',
    '_USAGE_GROUPS_USER' => '下方是該使用者所屬群組清單。勾選某群組旁核取方塊並按下「移除」可將該使用者自群組移除。',
    '_USAGE_GROUPS_LDAP' => '下方是 Movable Type 系統中的全部群組。勾選群組名稱旁核取方塊後按下「啟用」或「停用」可啟用/停用群組。',
    '_USAGE_GROUPS_USER_LDAP' => '下方是該使用者所屬群組清單。',
    '_USAGE_PLUGINS' => '這是目前已在 Movable Type 註冊的全部外掛清單。',
    '_USAGE_LIST_POWER' => '這裡是 [_1] 的批次編輯模式文章清單。您可以在下方表單修改目前顯示文章的任意值；完成後按下「儲存」。標準「列表與編輯文章」控制項（篩選、分頁）在批次模式下同樣可用。',
    '_USAGE_ENTRY_LIST_BLOG' => '這裡是 [_1] 的文章清單，您可進行篩選、管理與編輯。',
    '_USAGE_ENTRY_LIST_OVERVIEW' => '這裡是所有部落格的文章清單，您可進行篩選、管理與編輯。',
    '_USAGE_COMMENTERS_LIST' => '這裡是 [_1] 的所有已驗證留言者清單。您可在下方將留言者標記為信任或封鎖，或查看更多資訊。',
    '_USAGE_PING_LIST_BLOG' => '這裡是 [_1] 的 TrackBack 清單，您可進行篩選、管理與編輯。',
    '_USAGE_PING_LIST_OVERVIEW' => '這裡是所有部落格的 TrackBack 清單，您可進行篩選、管理與編輯。',
    '_USAGE_PING_LIST_ALL_WEBLOGS' => '這裡是所有部落格的 TrackBack ping 清單，您可進行篩選、管理與編輯。',
    '_USAGE_NOTIFICATIONS' => '這裡是希望在您發佈內容時收到通知的使用者清單。要新增使用者，請在下方輸入其電子郵件地址（URL 為選填）。要刪除使用者，請在下表勾選刪除並按下「刪除」按鈕。',
    '_USAGE_SEARCH' => '您可以使用「搜尋與取代」工具查找並可選擇取代 Movable Type 多種清單項目中的文字或資料。重要提示：執行取代時請務必小心，因為<b>無法復原</b>。',
    '_INDEX_INTRO' => '<p>若您正在安裝 Movable Type，建議先查看<a href="https://www.sixapart.com/movabletype/docs/mtinstall.html">安裝說明</a>，並執行<a rel="nofollow" href="mt-check.cgi">Movable Type 系統檢查</a>，確認系統符合需求。</p>',
    '_SHORT_MAY' => '5月',
    '_AUTO' => 1,
    '_DISPLAY_OPTIONS_SHOW' => '顯示',
    '_FILTER_FUTURE' => '未來',
    '__AUTHOR_STATUS' => '狀態',
    '__BLOG_COUNT' => '部落格數',
    '__PING_COUNT' => 'TrackBack 數',
    '__WEBSITE_BLOG_NAME' => '網站/部落格名稱',
    '__SSL_CERT_UPDATE' => '更新',
    '__SSL_CERT_INSTALL' => '安裝',
    '_ABOUT_PAGE_BODY' => '<p>這是一個「關於」頁面範例。（通常「關於」頁面會用於介紹個人或公司摘要。）</p><p>若在網頁中使用 <code>@ABOUT_PAGE</code> 標籤，該「關於」頁面會加入頁首與頁尾導覽清單。</p>',
    '_SAMPLE_PAGE_BODY' => '<p>這是一個網頁範例。</p><p>若在網頁中使用 <code>@ADD_TO_SITE_NAV</code> 標籤，該頁面會加入頁首與頁尾導覽清單。</p>',
    '_CONTENT_TYPE_BOILERPLATES' => '內容類型樣板',
...
## lib/MT/Blog.pm
	...
	'Theme' => '主题',
    '__ASSET_COUNT' => '資源數',
    '__INTEGER_FILTER_EQUAL' => '是',
    '__INTEGER_FILTER_NOT_EQUAL' => '否',
    '__PAGE_COUNT' => '頁面數',
...
## tmpl/admin2023/cms/edit_author.tmpl
    ...
    '_USAGE_PASSWORD_RESET' => '找回密碼',
    '_USER_DISABLED' => '禁用',
    '_USER_ENABLED' => '啟用',
    '_USER_PENDING' => '審核中',
    '_USER_STATUS_CAPTION' => '狀態',
    '_WARNING_PASSWORD_RESET_SINGLE' => '您即將重置“[_1]”的密碼。系統將隨機生成一個新密碼並直接發送到其電子郵寄地址 ([_2])。是否繼續？',
...
## tmpl/admin2023/cms/include/basic_filter_forms.tmpl
	...
	'[_1] and [_2]' => '[_1]和 [_2]',
	'[_1] hours' => '[_1] 小时',
    '_FILTER_DATE_DAYS' => '[_1] 天',
    '__FILTER_DATE_ORIGIN' => '[_1]',
    '__STRING_FILTER_EQUAL' => '是',
    '__TIME_FILTER_HOURS' => '最近',
	'contains' => 'contains',
