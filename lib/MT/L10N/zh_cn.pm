...
##修复日期格式问题
	...
	'__DATE_FILTER_FUTURE' => '过去',
	'AUTO DETECT' => '自动检测',
	'_BLOG_CONFIG_MODE_BASIC' => '基础模式',
	'_BLOG_CONFIG_MODE_DETAIL' => '详细模式',
	'_SEARCH_SIDEBAR' => '搜索',
	'_USAGE_REBUILD' => '<a href="javascript:void(0)" onclick="doRebuild()">重建</a>以将这些更改反映到您的公开站点。',
	'_USAGE_FORGOT_PASSWORD_1' => '您已请求找回 Movable Type 密码。系统已为您重置密码；新密码如下：',
	'_USAGE_FORGOT_PASSWORD_2' => '您可以通过下面的 URL 使用此新密码登录 Movable Type。登录后，请将密码更改为更便于记忆的内容。',
	'_BACKUP_DOWNLOAD_MESSAGE' => '导出文件将在几秒后自动开始下载。如果未自动开始，请点击<a href="javascript:(void)" onclick="submit_form()">这里</a>手动下载。请注意，每个会话只能下载一次导出文件。',
	'_USAGE_ARCHIVE_MAPS' => '此高级功能可让您将任意归档模板映射到多个归档类型。例如，您可能希望为月度归档创建两种不同视图：一种以列表形式展示该月条目，另一种以该月的日历形式展示条目。',
	'_USAGE_ARCHIVING_1' => '选择您希望在站点启用的归档频率/类型。对于每种归档类型，您都可以指定多个归档模板。例如，您可能希望为月度归档创建两种不同视图：一种页面列出该月所有条目，另一种以该月日历视图展示。',
	'_USAGE_ARCHIVING_2' => '当您为某种归档类型关联多个模板（甚至只关联一个模板）时，都可以使用归档文件模板自定义归档文件的输出路径。',
	'_USAGE_ARCHIVING_3' => '选择要添加新归档模板的归档类型，然后选择要与该归档类型关联的模板。',
	'_USAGE_BANLIST' => '下面列出了被您禁止在站点发表评论或发送 TrackBack 的 IP 地址。要添加新 IP，请在下方表单输入地址。要删除已封禁 IP，请在下表中勾选删除并点击“删除”按钮。',
	'_USAGE_PREFS' => '此页面可设置与博客、归档、评论以及发布与通知相关的可选项。创建新博客时，这些值会采用合理默认值。',
	'_USAGE_FEEDBACK_PREFS' => '此页面可配置读者向您的博客提交反馈的方式。',
	'_USAGE_PROFILE' => '在此编辑您的用户资料。如果更改用户名或密码，登录凭据会自动更新，您无需重新登录。',
	'_GENL_USAGE_PROFILE' => '在此编辑该用户资料。如果更改用户名或密码，该用户的登录凭据会自动更新，无需重新登录。',
	'_USAGE_GROUP_PROFILE' => '此页面可编辑用户组资料。',
	'_USAGE_NEW_AUTHOR' => '您可以在此页面创建系统新用户。',
	'_USAGE_NEW_GROUP' => '您可以在此页面创建系统新用户组。',
	'_USAGE_ROLES' => '您可以在此页面查看您在各博客中的角色并创建角色。点击角色名称可查看详细信息。',
	'_USAGE_ROLE_PROFILE' => '您可以在此页面定义角色及其权限。',
	'_USAGE_ASSOCIATIONS' => '您可以在此页面查看并创建权限关联。',
	'_USAGE_CATEGORIES' => '使用分类可更方便地组织条目，便于引用、归档与博客展示。创建或编辑条目时可指定分类。编辑已有分类请点击分类标题；创建子分类请点击对应“创建”按钮；移动分类请点击对应“移动”按钮。',
	'_USAGE_CATEGORY_PING_URL' => '这是其他人向您的博客发送 TrackBack 时使用的 URL。若希望任何人都可向该分类发送 TrackBack，请公开此 URL；若仅允许特定人群，请私下发送此 URL。若要在主索引模板中显示收到的 TrackBack 列表，请参阅与 TrackBack 相关的模板标签文档。',
	'_USAGE_TAGS' => '使用标签可更方便地组织条目，便于引用与博客展示。',
	'_USAGE_COMMENT' => '编辑所选评论。完成后点击“保存”。要使更改生效，您需要重建。',
	'_USAGE_PERMISSIONS_1' => '您正在编辑 <b>[_1]</b> 的权限。下方列出您有用户编辑权限的博客；请在每个博客中勾选要授予 <b>[_1]</b> 的访问权限。',
	'_USAGE_PERMISSIONS_2' => '若要编辑其他用户权限，请在下拉菜单中选择用户，然后点击“编辑”。',
	'_USAGE_PERMISSIONS_3' => '您可通过两种方式编辑用户并授予/撤销访问权限。快捷方式是从下方菜单选择用户并编辑；也可浏览完整用户列表，再选择要编辑或删除的用户。',
	'_USAGE_PERMISSIONS_4' => '每个博客可有多个用户。添加用户时，请在下方表单输入用户信息，然后选择该用户可访问的博客。点击“保存”并创建用户后，您可继续编辑其权限。',
	'_USAGE_PLACEMENTS' => '使用下方工具管理该条目的次级分类。左侧列表是该条目尚未设为主分类或次级分类的分类；右侧列表是该条目当前所属的次级分类。',
	'_USAGE_ENTRYPREFS' => '选择在条目编辑器中显示的字段集合。',
	'_USAGE_IMPORT' => '您可以从 Movable Type 安装目录下的 <code>import</code> 目录文件导入博客条目，也可以通过下方表单上传后导入。支持从其他 Movable Type 安装或其他应用导入。',
	'_USAGE_EXPORT_2' => '要导出条目，请点击下方链接（“从 [_1] 导出条目”）。若要将导出数据保存为文件，Mac 请按住 <code>option</code> 键，PC 请按住 <code>Shift</code> 键再点击链接。也可全选数据后复制到其他文档。（<a href="javascript:void(0)" onclick="openManual(\'importing\', \'export_ie\');return false;">从 Internet Explorer 导出？</a>）',
	'_USAGE_EXPORT_3' => '点击下方链接将把您当前所有博客条目导出到 Tangent 服务器。通常这是安装 Movable Type 的 Tangent 插件后的单次推送，但您也可在需要时执行。',
	'_USAGE_AUTHORS' => '这是 Movable Type 系统中的全部用户列表。点击用户名可编辑其资料。',
	'_USAGE_AUTHORS_1' => '这是 Movable Type 系统中的全部用户列表。点击用户名可编辑其资料。您还可通过基于 CSV 的命令文件创建、编辑和删除用户记录。',
	'_USAGE_AUTHORS_LDAP' => '这是 Movable Type 系统中的全部用户列表。点击用户名可编辑其资料。勾选用户旁复选框并点击“禁用”可禁用该用户，禁用后其将无法登录 Movable Type。',
	'_USAGE_AUTHORS_2' => '您可以上传包含命令及相关数据的 CSV 格式文件，以批量创建、编辑和删除用户。',
	'_USAGE_GROUPS' => '下方是 Movable Type 系统中的全部用户组。勾选组名旁复选框后点击“启用”或“禁用”可启用/禁用用户组；点击组名可编辑用户组。',
	'_USAGE_GROUPS_USER' => '下方是该用户所属的用户组列表。勾选某个用户组旁复选框并点击“移除”可将该用户从组中移除。',
	'_USAGE_GROUPS_LDAP' => '下方是 Movable Type 系统中的全部用户组。勾选组名旁复选框后点击“启用”或“禁用”可启用/禁用用户组。',
	'_USAGE_GROUPS_USER_LDAP' => '下方是该用户所属的用户组列表。',
	'_USAGE_PLUGINS' => '这是当前已在 Movable Type 注册的全部插件列表。',
	'_USAGE_LIST_POWER' => '这里是 [_1] 的批量编辑模式条目列表。您可以在下方表单修改当前显示条目的任意值；完成后点击“保存”。标准“列表与编辑条目”控件（筛选、分页）在批量模式下同样可用。',
	'_USAGE_ENTRY_LIST_BLOG' => '这里是 [_1] 的条目列表，您可进行筛选、管理和编辑。',
	'_USAGE_ENTRY_LIST_OVERVIEW' => '这里是所有博客的条目列表，您可进行筛选、管理和编辑。',
	'_USAGE_COMMENTERS_LIST' => '这里是 [_1] 的所有已认证评论者列表。您可在下方将评论者标记为受信任或封禁，或查看更多信息。',
	'_USAGE_PING_LIST_BLOG' => '这里是 [_1] 的 TrackBack 列表，您可进行筛选、管理和编辑。',
	'_USAGE_PING_LIST_OVERVIEW' => '这里是所有博客的 TrackBack 列表，您可进行筛选、管理和编辑。',
	'_USAGE_PING_LIST_ALL_WEBLOGS' => '这里是所有博客的 TrackBack ping 列表，您可进行筛选、管理和编辑。',
	'_USAGE_NOTIFICATIONS' => '这里是希望在您发布内容时收到通知的用户列表。要添加用户，请在下方输入其邮箱地址（URL 可选）。要删除用户，请在下表勾选删除并点击“删除”按钮。',
	'_USAGE_SEARCH' => '您可以使用“搜索与替换”工具查找并可选择替换 Movable Type 多种列表项中的文本或数据。重要提示：执行替换时请谨慎，因为<b>无法撤销</b>。',
	'_INDEX_INTRO' => '<p>如果您正在安装 Movable Type，建议先查看<a href="https://www.sixapart.com/movabletype/docs/mtinstall.html">安装说明</a>，并运行<a rel="nofollow" href="mt-check.cgi">Movable Type 系统检查</a>，确认系统满足运行要求。</p>',
	'_SHORT_MAY' => '5月',
	'_AUTO' => 1,
	...
	'__AUTHOR_STATUS' => '状态',
	'__BLOG_COUNT' => '博客数',
	'__PING_COUNT' => 'TrackBack 数',
	'__WEBSITE_BLOG_NAME' => '网站/博客名称',
	'__SSL_CERT_UPDATE' => '更新',
	'__SSL_CERT_INSTALL' => '安装',
	'_ABOUT_PAGE_BODY' => '<p>这是一个“关于”页面示例。（通常“关于”页面用于介绍个人或公司概况。）</p><p>如果在网页中使用了 <code>@ABOUT_PAGE</code> 标签，该“关于”页面会被加入页眉和页脚的导航列表。</p>',
	'_SAMPLE_PAGE_BODY' => '<p>这是一个网页示例。</p><p>如果在网页中使用了 <code>@ADD_TO_SITE_NAV</code> 标签，该页面会被加入页眉和页脚的导航列表。</p>',
	'_CONTENT_TYPE_BOILERPLATES' => '内容类型样板',
...
## tmpl/admin2023/cms/edit_author.tmpl
	...
	'_USAGE_PASSWORD_RESET' => '找回密码',
	'_USER_DISABLED' => '禁用',
	'_USER_ENABLED' => '启用',
	'_USER_PENDING' => '审核中',
	'_USER_STATUS_CAPTION' => '状态',
	'_WARNING_PASSWORD_RESET_SINGLE' => '您即将重置“[_1]”的密码。系统将随机生成一个新密码并直接发送到其电子邮件地址 ([_2])。是否继续？',
...
## tmpl/admin2023/cms/list_theme.tmpl
	...
	'_THEME_DIRECTORY_URL' => 'https://themes.movabletype.org/',
